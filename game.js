...
// สุ่มไอเท็มผลึกคริสตัล
function spawnCrystal() {
  const r = Math.random();
  let color = "#38bdf8", value = 20;
  if (r > 0.85) { color = "#fbbf24"; value = 70; }
  return {
...
function executeHealing() {
  if (state.player.gold >= 50) {
    ...
    const healHP = Math.floor(state.player.maxHp * 0.45); 
    state.player.hp = Math.min(state.player.maxHp, state.player.hp + healHP);
    state.player.mana = Math.min(state.player.maxMana, state.player.mana + 25);
...
// คลิกตรวจพิกัดบน Grid เมนู
canvas.addEventListener("mousedown", (e) => {
  const rect = canvas.getBoundingClientRect();
  // คำนวณพิกัดเมาส์ให้ตรงกับความละเอียด Canvas 960x600 เสมอ แม้ขยายเต็มจอ
...
// --- 🖥️ ระบบควบคุม Fullscreen (หน้าจอเต็ม) ที่เสถียร 🖥️ ---
function toggleFullscreen() {
  if (!document.fullscreenElement) {
    // ให้องค์กรหลัก .game-shell หรือตัว canvas แสดงผลเต็มจอ
    canvas.requestFullscreen().catch(err => {
      console.warn(`เข้าโหมดเต็มจอไม่ได้: ${err.message}`);
    });
  } else {
